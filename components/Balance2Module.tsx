  ...
  function RefCell({ children, refId, className = "" }: { children: React.ReactNode; refId: string; className?: string }) {
    return (
      <td
        ...
        className={`whitespace-nowrap px-1 py-0.5 text-center text-[10px] font-black ${insertingReference ? "cursor-copy ring-1 ring-blue-100 hover:bg-blue-100" : ""} ${className}`}
  ...
  function EditCell({ row, field, label }: { row: CalculatedRow; field: EditField; label?: string }) {
    ...
    return (
      <td className="min-w-[62px] px-1 py-0.5">
        {label ? <div className="mb-0.5 text-center text-[10px] font-bold text-slate-400">{label}</div> : null}
        <input
          type="text"
          inputMode="decimal"
          readOnly
          ...
          className={`h-7 w-full cursor-pointer rounded-lg border px-1 text-center text-[10px] font-black outline-none transition ${isActive ? "border-[#0057B8] ring-2 ring-blue-100 bg-white text-slate-950" : "border-blue-100 bg-slate-50 text-slate-900 hover:border-blue-300"}`}
              ...
              {rows.map((row, index) => {
                ...
                return (
                  <tr key={row.id} className="border-b border-slate-100 hover:bg-blue-50/70">
                    <td className="whitespace-nowrap px-2 py-0.5 text-[10px] font-black text-slate-950 text-center">{row.codigo}</td>
                    ...
                    {semanasActivas.map((sem) => (
                      <td key={`${row.id}-need-${sem}`} className="whitespace-nowrap px-1 py-0.5 text-center text-[10px] font-black text-red-600">{formato(row.necesidadesPorSemana[sem] || 0)}</td>
                    ))}
...
function GroupTh({ children, colSpan }: { children: React.ReactNode; colSpan: number }) {
  return <th colSpan={colSpan} className="border-l border-blue-300 px-1 py-1 text-center text-[10px] font-black uppercase tracking-wide">{children}</th>;
}

function Th({ children, right = false, rowSpan }: { children: React.ReactNode; right?: boolean; rowSpan?: number }) {
  return <th rowSpan={rowSpan} className={`whitespace-nowrap border-l border-blue-200 px-1 py-1 text-[10px] font-black uppercase text-center`}>{children}</th>;
}
